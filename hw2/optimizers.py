...
class VanillaSGD(Optimizer):
    ...
    def step(self):
        for p, dp in self.params:
            ...
            # ====== YOUR CODE: ======
            # adding reguralization term to the grad
            dp += self.reg * p
            # print("norm!!!", dp.norm())
...
class MomentumSGD(Optimizer):
    def __init__(self, params, learn_rate=1e-3, reg=0, momentum=0.9):
        ...
        # ====== YOUR CODE: ======
        self.v = [torch.zeros_like(p) for p, dp in self.params]
    ...
    def step(self):
        ...
        for p, dp in self.params:
            ...
            self.v[i] -= self.learn_rate * dp
            # print("norm!!!", dp.norm())
            # print("sizes: ", p.size(), self.v[i].size())
            p += self.v[i]
            # print("weigth: ", p.norm())
            i += 1
            # ========================
...
class RMSProp(Optimizer):
    def __init__(self, params, learn_rate=1e-3, reg=0, decay=0.99, eps=1e-8):
        ...
        # TODO: Add your own initializations as needed.
        # ====== YOUR CODE: ======
        self.r = [torch.ones_like(p) for p, dp in self.params]
        # ========================
    ...
    def step(self):
        ...
        for p, dp in self.params:
            ...
            dp += self.reg * p
            self.r[i] *= self.decay
            self.r[i] += (1 - self.decay) * (dp * dp)
            # print("norm!!!", dp.norm())
            # print("sizes: ", p.size(), self.v[i].size())
            p -= (self.learn_rate / (self.r[i] + self.eps).sqrt()) * dp
            # print("weigth: ", p.norm())
            i += 1
            # ========================
