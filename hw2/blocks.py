...
class Linear(Block):
    ...
    def forward(self, x, **kw):
        ...
        # ====== YOUR CODE: ======
        out = x.mm(torch.t(self.w)) + self.b
        # ========================
    ...
    def backward(self, dout):
        ...
        dx = dout.mm(self.w)
        self.dw += dout.t().mm(x)
        # print("Sizes: ", (torch.ones((1, dout.size()[0])).mm(dout)).size(), self.db.size())
        self.db += (torch.ones((1, dout.size()[0])).mm(dout)[0, :])
        # print("dirivatives", self.dw.norm(), self.db.norm())
        # ========================
...
class CrossEntropyLoss(Block):
    ...
    def forward(self, x, y):
        ...
        vec1 = x[range(N), y]
        # print(vec1.size())
        vec2 = x.exp().sum(dim=1)

        # for grad calculations
        # print(x.size())
        # print(vec2)
        vec3 = vec2.view((N, 1))
        vec3 = vec3.expand(N, x.size()[1])
        # print(vec3)
        # print(vec3.size())
        self.grad_cache['soft_max'] = x.exp() / vec3
        # print(self.grad_cache['soft_max'])
        # print(self.grad_cache['soft_max'].sum(dim=1))

        vec1 = ((-1) * vec1) + vec2.log()
        loss = (1 / N) * vec1.sum()
        # ========================
    ...
    def backward(self, dout=1.0):
        ...
        # ====== YOUR CODE: ======
        select_x = torch.zeros_like(x)
        select_x[range(N), y] = 1
        dx = dout * (self.grad_cache['soft_max'] - select_x) / N
        # ========================
